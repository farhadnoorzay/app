...
class FlexUnlimited:
  ...
  def __getFlexAccessToken(self):
    ...
    try:
        response = self.session.post(FlexUnlimited.routes.get("RequestNewAccessToken"), json=data, headers=headers).json()
        self.accessToken = response['access_token']

        # Update the access token in the request headers
        self.__requestHeaders["x-amz-access-token"] = self.accessToken

        # Update the refresh token in your class attribute
        self.refreshToken = response['refresh_token']

        # Update the config file or wherever you store the tokens
        with open("config.json", "r+") as configFile:
            config = json.load(configFile)
            config["accessToken"] = self.accessToken
            config["refreshToken"] = self.refreshToken
        ...
        Log.error(f"Failed to refresh access token: {str(e)}")
    except KeyError as nullKey:
      Log.error(f'{nullKey} was not set. Please setup FlexUnlimited as described in the README.')
      sys.exit()
    except FileNotFoundError:
  ...
  def run(self):
    ...
    while self.__retryCount < self.retryLimit:
      ...
      if offersResponse.status_code == 200:
        ...
        retry_after = int(offersResponse.headers.get('Retry-After', 30))
        Log.info(f"Rate limit reached. Waiting for {retry_after} seconds.")
